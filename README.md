...
The system supports category sharing, allowing tasks within a category to be accessed by other users with appropriate permissions. Real-time synchronization is implemented using Socket.io, enabling live updates across clients without page reloads. Data persistence is handled through a relational database namely MySQL via xampp, with structured tables for users, tasks, and sharing relationships. The data was previously stored in json files but sent over to MySQL using the models and migrate.js to automatically transfer all the data. 
